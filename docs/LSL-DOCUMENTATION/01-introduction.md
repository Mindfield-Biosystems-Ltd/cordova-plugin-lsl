...
> Source: https://labstreaminglayer.readthedocs.io/info/intro.html
...
The core transport library is [liblsl](https://github.com/labstreaminglayer/liblsl/) and its language interfaces ([C](https://github.com/sccn/liblsl/), [C++](https://github.com/sccn/liblsl/), [Python](https://github.com/labstreaminglayer/liblsl-Python/), [Java](https://github.com/labstreaminglayer/liblsl-Java/), [C#](https://github.com/labstreaminglayer/liblsl-Csharp/), [MATLAB](https://github.com/labstreaminglayer/liblsl-Matlab/)).
The library is general-purpose and cross-platform (OS Support: Win / Linux / MacOS / [Android](https://github.com/labstreaminglayer/liblsl-Android/) / iOS; Architecture Support: x86 / amd64 / arm).

The suite of tools includes a [recording program](https://github.com/labstreaminglayer/App-LabRecorder),
[file importers](https://github.com/sccn/xdf), and apps that make data from
a range of acquisition hardware (see [Supported Devices and Tools](https://labstreaminglayer.readthedocs.io/info/supported_devices.html)) available on the
lab network (for example audio, EEG, or motion capture).

There is an [intro lecture/demo on LSL](http://www.youtube.com/watch?v=Y1at7yrcFW0) (part of an online course on EEG-based brain-computer interfaces).
...
**Sample**
: A single measurement of all channels from a device is called a sample.

**Chunk**
: A Sample can be transferred by itself for improved latency or in chunks of multiple samples for improved throughput.

**Metadata**
: Apart from the raw data, information about the Stream is stored and transmitted as XML data (akin to a file header).

**Stream**
: The combination of sampled data from a device with the Metadata is called a stream.
A stream can have a regular sampling rate (e.g. audio sampled at 44100 Hz, videos at 24 or 60 Hz)
or an irregular sampling rate (e.g. key presses, experimental events) and one or more channels
(e.g. two channels for stereo audio, 32 / 64 channels for EEG recordings,
1920*1080 channels for a full HD screen capture, one channel for key presses).
...
subscribe to it by finding it via a Resolver and connecting a Stream Inlet to it.

**Stream Inlet**
: A stream inlet is for receiving time series data from a single connected outlet.
Allows to retrieve samples from the Stream (in-order, with reliable
(re-)transmission, optional type conversion and optional failure recovery).
Besides the samples, the Metadata can be obtained (as XML blob
or alternatively through a small built-in DOM interface).

**Resolver**
: LSL provides functions to resolve streams that are
...
synchronized. See [Time Synchronization](https://labstreaminglayer.readthedocs.io/info/time_synchronization.html).
...
This feature is explained in more detail in the [Time Synchronization](https://labstreaminglayer.readthedocs.io/info/time_synchronization.html) document.

### File Format

The transport API itself does not endorse or provide a particular file format,
but the provided recording program ([https://github.com/labstreaminglayer/App-LabRecorder](https://github.com/labstreaminglayer/App-LabRecorder)) records into the
XDF file format ([Extensible Data Format](https://github.com/sccn/xdf)).
...
Please see the separate developer documentation [Developer's Guide](https://labstreaminglayer.readthedocs.io/dev/dev_guide.html).
