...
> Source: https://labstreaminglayer.readthedocs.io/info/lslapicfg.html
...
- `/etc/lsl_api/lsl_api.cfg` (Unix);
`C:\etc\lsl_api\lsl_api.cfg` (Windows).
This is a global directory that is visible to all applications from all users
on a given computer.

- `~/lsl_api/lsl_api.cfg` (Unix);
`%HOMEPATH%\lsl_api\lsl_api.cfg` (Windows).
...
- **LSLAPICFG** environment variable:
the file specified in the environment variable `LSLAPICFG`.
This is a shell-specific location.
On Linux / OS X you can set the env variable either before the program
invocation (`LSLAPICFG=/tmp/specialconf1.cfg ./LabRecorder`) or as
an export (`export LSLAPICFG=/tmp/specialconf1.cfg`) to apply it to
...
[https://github.com/sccn/liblsl/blob/master/src/api_config.cpp#L84](https://github.com/sccn/liblsl/blob/master/src/api_config.cpp#L84)
...
In a local lab, the `lab.KnownPeers` option is usually a better choice, though.
