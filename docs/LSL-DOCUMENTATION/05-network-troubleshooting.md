...
> Source: https://labstreaminglayer.readthedocs.io/info/network-connectivity.html
...
the example programs, e.g. `labstreaminglayer/LSL/liblsl/bin/SendDataC`
and `labstreaminglayer/LSL/liblsl/bin/ReceiveDataC`, respectively.
...
Start Menu -> Control Panel -> Windows Firewall -> Turn Windows Firewall on or off
...
Home or work (private) networks will be listed as "Not Connected"
and instead your current connection will show up under "Public networks".
...
such a network, and therefore cannot safely use LSL on a public wifi.
...
Under View your active networks you should see your current
network, incorrectly labeled as "public network".
...
Hit `Alt` to bring up the menu and select
...
([https://github.com/sccn/liblsl/pull/31](https://github.com/sccn/liblsl/pull/31)).

You can also eliminate Hyper-V altogether by going to Control Panel -> Programs and Features -> Turn Windows features on or off and disabling Hyper-V. This requires admin privileges and a reboot.
...
For more information on these advanced settings, please read the following StackExchange article: [https://superuser.com/questions/1282014/how-to-remove-all-the-vethernet-default-switch-once-and-for-all](https://superuser.com/questions/1282014/how-to-remove-all-the-vethernet-default-switch-once-and-for-all).
...
To log _all_ broadcast packets, use
`UDP-RECV:16571,reuseaddr,broadcast` as source.

For a specific broadcast address
`UDP-RECV:16571,reuseaddr,bind=224.0.0.183,ip-add-membership=224.0.0.183:enp0s25`
where `224.0.0.183` is the broadcast address and (optionally) `enp0s25`
your interface name.

For multicast packets, use
`UDP6-RECV:16571,reuseaddr,ipv6-join-group='[ff02:113D:6FDD:2C17:A643:FFE2:1BD1:3CD2]:enp0s25'`.
Note that the interface name may also be an interface index as printed by
`ip addr` (here `wlan0` has the interface index 4):
...
using an appropriately-placed [configuration file](https://labstreaminglayer.readthedocs.io/info/lslapicfg.html).
...
Distribute this configuration file between the outlet and inlet and _make sure the
directory and file names are correct per your OS distribution._ **If the name of the file is
not exactly `lsl_api.cfg`, liblsl will not read it.**

Finally to test that a remote connection is established, use an outlet/inlet
test using example files such as:

- [https://github.com/chkothe/pylsl/blob/master/examples/SendData.py](https://github.com/chkothe/pylsl/blob/master/examples/SendData.py)
- [https://github.com/chkothe/pylsl/blob/master/examples/ReceiveData.py](https://github.com/chkothe/pylsl/blob/master/examples/ReceiveData.py)
...
[https://www.yougetsignal.com/tools/open-ports/](https://www.yougetsignal.com/tools/open-ports/). The port will only appear open when an
...
wireless networks. Please see the [tuning] section at the bottom of the [LSL configuration files](https://labstreaminglayer.readthedocs.io/info/lslapicfg.html)
