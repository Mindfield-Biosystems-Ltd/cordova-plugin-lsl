# liblsl api_config.cpp - Configuration Source

> Source: https://github.com/sccn/liblsl/blob/main/src/api_config.cpp
> 293 lines (265 loc) - 11 KB
...
void api_config::load_from_file(const std::string &filename) {
    try {
        ...
        // Multicast groups in the 239.192.0.0/14 subnet (RFC2365)
...
1. `LSLAPICFG` environment variable
2. `lsl_api.cfg` (current directory)
3. `~/lsl_api/lsl_api.cfg` (home directory)
4. `/etc/lsl_api/lsl_api.cfg` (system-wide)

### [ports] Section
| Parameter | Default | Description |
|-----------|---------|-------------|
| MulticastPort | 16571 | Port for multicast discovery |
| BasePort | 16572 | Base port for TCP/UDP connections |
| PortRange | 32 | Number of ports to scan |
| AllowRandomPorts | true | Allow random port assignment |
| IPv6 | "allow" | IPv6 mode: "disable", "allow", "force" |

### [multicast] Section
| Parameter | Default | Description |
|-----------|---------|-------------|
| ResolveScope | "site" | Discovery scope: machine/link/site/organization/global |
| ListenAddress | "" | Specific listen address |
| MachineAddresses | {127.0.0.1} | Loopback addresses |
| LinkAddresses | {255.255.255.255, 224.0.0.1, 224.0.0.183} | Link-local multicast |
| SiteAddresses | {239.255.172.215} | Site-local multicast |

### [tuning] Section
| Parameter | Default | Description |
|-----------|---------|-------------|
| WatchdogCheckInterval | 15.0 | Watchdog check interval (seconds) |
| WatchdogTimeThreshold | 15.0 | Watchdog time threshold (seconds) |
| MulticastMinRTT | 0.5 | Minimum multicast round-trip time |
| MulticastMaxRTT | 3.0 | Maximum multicast round-trip time |
| UnicastMinRTT | 0.75 | Minimum unicast round-trip time |
| UnicastMaxRTT | 5.0 | Maximum unicast round-trip time |
| ContinuousResolveInterval | 0.5 | Continuous resolve interval |
| TimeUpdateInterval | 2.0 | Time sync update interval |
| TimeProbeCount | 8 | Number of time probes |
| TimeProbeInterval | 0.064 | Interval between time probes |
| TimeProbeMaxRTT | 0.128 | Maximum RTT for time probes |
| OutletBufferReserveMs | 5000 | Outlet buffer reserve (ms) |
| OutletBufferReserveSamples | 128 | Outlet buffer reserve (samples) |
| InletBufferReserveMs | 5000 | Inlet buffer reserve (ms) |
| InletBufferReserveSamples | 128 | Inlet buffer reserve (samples) |
| SmoothingHalftime | 90.0 | Clock smoothing half-time |
| ForceDefaultTimestamps | false | Force default timestamps |

### [lab] Section
| Parameter | Default | Description |
|-----------|---------|-------------|
| KnownPeers | {} | Known peer addresses for direct resolution |
| SessionID | "default" | Lab session identifier |
